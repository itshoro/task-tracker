...
import { RadioPillGroup } from "@/app/components/radio";
...
const ColorSchemeSelect = () => {
  const [pending, theme, setTheme] = usePersistedState<ColorScheme>(
    "theme",
    "system",
    undefined,
    true
  ...
  return (
    <RadioPillGroup
      ...
      name="color-scheme"
      defaultValue={theme}
      tabs={colorSchemes.map((scheme) => ({
        value: scheme,
        children: (
          <>
            {schemeIcons[scheme]} {scheme}
          </>
        ),
      }))}
      onChange={(e) => {
